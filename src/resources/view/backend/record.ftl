<#if deliveryResumeList?? && deliveryResumeList.result??>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td align="center" valign="middle" bgcolor="#ffffff"  class="caozuo"><a href="/website/jobDetail?jobs.id=${t.jobs.id!}" target="blank">查看</a>&nbsp;&nbsp;  <a href="/backend/manageCenter/delete?dr.id=${t.id!}" onclick="javascript:if(confirm('确认要删除该记录吗?')==false) return false;">删除</a></td>
          </tr>
          </#if>
          </#list>
          </#if>
...
<#if deliveryResumeList?? && deliveryResumeList.result?? && deliveryResumeList.result?size gt 0>
	...
	<#if pageNo gt 1>
	<a data-class="p1" href="/backend/manageCenter?pageNo=${pageNo-1}">上一页</a>
	<#else>
	...
	<span>${t!}</span>
	<#else>
	<a href="/backend/manageCenter?pageNo=${t!}">${t!}</a>
	</#if>
	...
	<a data-class="n1" href="/backend/manageCenter?pageNo=${pageNo+1}">下一页</a>
	<#else>
...
form_data={};
form_data.ids = ids;
var url  = "/backend/manageCenter/delall";
$.ajax({
	 type: "POST",
     url: url,
     data: form_data,
...
<script>
	...
	var tarUrl = "/backend/manageCenter?";
